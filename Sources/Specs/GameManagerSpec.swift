...
let GameManagerSpec = describe("game manager"){
  ...
  var ui: UI = UI(io: io)
  var gameManager = GameManager(player1: player1, player2: player2, board: board, endGameCriteria: endGameCriteria, ui: ui)
  ...
  before {
    ...
    io = TestIO(inputs:[])
    ui = UI(io: io)
    gameManager = GameManager(player1: player1, player2: player2, board: board, endGameCriteria: endGameCriteria, ui: ui)
