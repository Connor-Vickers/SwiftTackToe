...
let GameCriteriaSpec = describe("gameCriteria") {
  var board: Board = Board()
  var gameCriteria: GameCriteria = StandardTTT()
  ...
  it("recongnizes a row win") {
    ...
    expect(gameCriteria.isWin(board: board)).to(equal: true)
  }
  ...
  it("blank board is not a win") {
    expect(gameCriteria.isWin(board: board)).to(equal: false)
  }
  it("blank board is not a tie") {
    expect(gameCriteria.isTie(board: board)).to(equal: false)
  }
  it("recongnizes a tie") {
    ...
    expect(gameCriteria.isTie(board: board)).to(equal: true)
  }
}